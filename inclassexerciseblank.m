...
CC = bwconncomp(data_segmented);
stats = regionprops(CC, 'Area');
area = (stats.Area);
s = round(1.2*sqrt(mean(area))/pi);
eroded = imerode(data_segmented, strel('disk',s));
outside = ~imdilate(data_segmented, strel('disk',1));
...
%When using Ilastik, the hysteresis threshold filters makes the cells
%appear more defined, as opposed to the previous watershed method, wherein
%it was more difficult to differentiate between individual ones.
